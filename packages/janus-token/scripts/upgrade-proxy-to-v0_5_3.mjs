/**
 * upgrade-proxy-to-v0_5_3.mjs — UUPS upgrade proxy to JanusFlow_v0_5_3.
 *
 * Prerequisites:
 *   1. node scripts/deploy-impl-v0_5_3.mjs  (creates deployments/janusflow-v0_5_3.json)
 *   2. npx hardhat compile                  (artifacts must exist)
 *
 * This script:
 *   1. Reads impl_address from deployments/janusflow-v0_5_3.json
 *   2. Calls proxy.upgradeToAndCall(newImpl, "0x") via the owner COA
 *   3. Verifies the ERC1967 Upgraded event
 *   4. Spot-checks VERSION and firstSnapshotBlock ABI on the proxy
 *   5. Updates deployments/janusflow-v0_5_3.json with upgrade tx hash
 *
 * Run from package root:
 *   node scripts/upgrade-proxy-to-v0_5_3.mjs
 */
...
const DEPLOY_RECORD = join(DEPLOYMENTS_DIR, "janusflow-v0_5_3.json");
...
async function main() {
    ...
    console.log("JanusFlow v0.5.3 — Upgrade proxy");
    console.log("=".repeat(72));

    if (!existsSync(DEPLOY_RECORD)) {
        throw new Error(`Missing deploy record: ${DEPLOY_RECORD} — run deploy-impl-v0_5_3.mjs first`);
    }
    ...
    // ─── 1. UUPS upgrade ────────────────────────────────────────────────────
    console.log("\n[1/3] Calling proxy.upgradeToAndCall(newImpl, 0x)...");
    const upgradeCalldata = jfIface.encodeFunctionData("upgradeToAndCall", [implAddr, "0x"]);
    const upgradeRes  = runFlowTx(CALL_TX, [PROXY, upgradeCalldata.slice(2)], "v053_upgrade_proxy");
    ...
    // ─── 2. Verify ERC1967 event ─────────────────────────────────────────────
    console.log("\n[2/3] Verifying upgrade event...");
    ...
    // ─── 3. Spot-check VERSION and firstSnapshotBlock ────────────────────────
    console.log("\n[3/3] Spot-checking proxy state...");
    ...
    const versionOk = versionStr === "0.5.3";
    console.log(`  proxy.VERSION() = "${versionStr}" → ${versionOk ? "OK" : "FAIL"}`);
    if (!versionOk) throw new Error(`VERSION mismatch: expected "0.5.3", got "${versionStr}"`);

    // firstSnapshotBlock for a fresh address should be 0
    const ZERO_ADDR = "0x0000000000000000000000000000000000000001";
    const fsbData = await provider.call({
        to: PROXY,
        data: jfIface.encodeFunctionData("firstSnapshotBlock", [ZERO_ADDR]),
    });
    const [fsb] = jfIface.decodeFunctionResult("firstSnapshotBlock", fsbData);
    const fsbOk = BigInt(fsb) === 0n;
    console.log(`  proxy.firstSnapshotBlock(zero) = ${fsb} → ${fsbOk ? "OK" : "FAIL"}`);
    if (!fsbOk) throw new Error("firstSnapshotBlock unexpectedly non-zero for fresh address");
    ...
    record.verification    = {
        ...
        firstSnapshotBlock_fresh_zero: fsbOk,
    };
    record.explorer = {
    ...
    console.log("\n" + "=".repeat(72));
    console.log("JanusFlow v0.5.3 upgrade COMPLETE");
    console.log(`Proxy preserved: ${PROXY}`);
    console.log(`New impl:        ${implAddr}`);
    console.log("New feature:     firstSnapshotBlock mapping active");
    console.log("=".repeat(72));
    return record;
}
