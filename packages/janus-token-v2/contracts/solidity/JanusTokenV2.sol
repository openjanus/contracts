...
//
// JanusTokenV2.sol — ElGamal Accumulator-based confidential token
//
...
//
// This is the core EVM contract for Phase 3 of openjanus v2.
// The Cadence layer (JanusFlowV2.cdc) wraps this contract for Flow-native UX.
//
...
//   JanusTokenV2 (this):      [PENDING — set after deploy]
//
// Improvements over phase 1 ElGamalAccumulator:
...
contract JanusTokenV2 {
    ...
    constructor(
        ...
        require(_babyJub != address(0), "JanusTokenV2: zero babyJub");
        require(_encryptVerifier != address(0), "JanusTokenV2: zero encryptVerifier");
        require(_decryptVerifier != address(0), "JanusTokenV2: zero decryptVerifier");
    ...
    function registerPubkey(uint256 x, uint256 y) external {
        require(!hasPubkey[msg.sender], "JanusTokenV2: pubkey already registered, use rotatePubkey");
        _validatePubkey(x, y);
    ...
    function commitPubkeyRotation(uint256 newX, uint256 newY) external {
        require(hasPubkey[msg.sender], "JanusTokenV2: not registered");
        _validatePubkey(newX, newY);
        require(
            newX != pubkey[msg.sender].x || newY != pubkey[msg.sender].y,
            "JanusTokenV2: new pubkey same as current"
        );
    ...
    function finalizePubkeyRotation() external {
        ...
        require(availableAt != 0, "JanusTokenV2: no pending rotation");
        require(block.timestamp >= availableAt, "JanusTokenV2: timelock not elapsed");
     ...
     * @dev Called by the Cadence layer (JanusFlowV2) which holds the actual
    ...
    function wrap(
        ...
        require(hasPubkey[to], "JanusTokenV2: recipient has no pubkey");
        require(msg.value > 0, "JanusTokenV2: must wrap nonzero amount");

        // Replay protection: nonce must match and increments
        require(senderNonce == nonce[msg.sender], "JanusTokenV2: invalid nonce");
        nonce[msg.sender]++;
    ...
    function confidentialTransfer(
        ...
        require(hasPubkey[msg.sender], "JanusTokenV2: sender not registered");
        require(hasPubkey[to], "JanusTokenV2: recipient not registered");
        require(transferAmount > 0, "JanusTokenV2: zero transfer");
        require(locked[msg.sender] >= transferAmount, "JanusTokenV2: insufficient locked balance");
    ...
    function unwrap(
        ...
        require(hasPubkey[msg.sender], "JanusTokenV2: not registered");
        require(amount > 0, "JanusTokenV2: zero amount");
        require(locked[msg.sender] >= amount, "JanusTokenV2: amount exceeds locked balance");

        // Verify decrypt_open proof
        // publicInputs[6] must equal amount (claimed_value)
        require(
            publicInputs[6] == amount,
            "JanusTokenV2: claimed_value in proof must match amount"
        );
        ...
        require(ok, "JanusTokenV2: FLOW transfer failed");
    ...
    function pubkeyOf(address user) external view returns (uint256 x, uint256 y) {
        require(hasPubkey[user], "JanusTokenV2: no pubkey registered");
    ...
    function _validatePubkey(uint256 x, uint256 y) internal view {
        require(x < P && y < P, "JanusTokenV2: pubkey coord out of field");
        require(!(x == 0 && y == 1), "JanusTokenV2: pubkey cannot be identity");
        require(babyJub.isOnCurve(x, y), "JanusTokenV2: pubkey not on BabyJub curve");
    }

    function _validateCiphertext(Ciphertext calldata ct) internal view {
        require(ct.C1x < P && ct.C1y < P && ct.C2x < P && ct.C2y < P,
            "JanusTokenV2: ciphertext coords out of field");
        require(babyJub.isOnCurve(ct.C1x, ct.C1y), "JanusTokenV2: C1 not on curve");
        require(babyJub.isOnCurve(ct.C2x, ct.C2y), "JanusTokenV2: C2 not on curve");
    }
    ...
    function _verifyEncryptProof(
        ...
        require(
            ...
            "JanusTokenV2: proof pubkey mismatch"
        );
        require(
            publicInputs[2] == ct.C1x && publicInputs[3] == ct.C1y,
            "JanusTokenV2: proof C1 mismatch"
        );
        require(
            publicInputs[4] == ct.C2x && publicInputs[5] == ct.C2y,
            "JanusTokenV2: proof C2 mismatch"
        );
        ...
        require(valid, "JanusTokenV2: encrypt proof invalid");
    }
    ...
    function _verifyDecryptProof(
        ...
        require(
            ...
            "JanusTokenV2: decrypt proof pubkey mismatch"
        );
        require(
            publicInputs[2] == cur.C1x && publicInputs[3] == cur.C1y,
            "JanusTokenV2: decrypt proof C1 mismatch"
        );
        require(
            publicInputs[4] == cur.C2x && publicInputs[5] == cur.C2y,
            "JanusTokenV2: decrypt proof C2 mismatch"
        );
        ...
        require(valid, "JanusTokenV2: decrypt proof invalid");
    }
